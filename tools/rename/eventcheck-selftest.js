...
const CASES = [
	...
	*/
	...
	['renommage coherent des deux cotes : rien ne doit apparaitre',
		{ rien: true },
		[['player.js', /"player-paused"/g, '"player-pause-toggled"']]],
];
...
try {
	...
	CASES.forEach(([label, expected, edits], i) => {
		...
		for (const [file, re, to] of edits) {
			...
			if (re === null) { fs.writeFileSync(p, to, 'utf8'); continue; }
			const before = fs.readFileSync(p, 'utf8');
			const after = before.replace(re, to);
			if (after === before) {
				console.log('  INVALIDE  ' + (i + 1) + '. ' + label + ' — ' + file + ' ne contient plus ' + re);
				failures++;
				return;
			}
