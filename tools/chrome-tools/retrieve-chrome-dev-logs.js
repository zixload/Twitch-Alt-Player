...
function collectLogs(target) {
    return new Promise((resolve) => {
        ...
        ws.on('message', (data) => {
            ...
            // Extract timestamp if available, default to now
            let ts = Date.now();
            if (msg.method === 'Runtime.consoleAPICalled' && msg.params.timestamp) {
                ts = msg.params.timestamp;
            } else if (msg.method === 'Log.entryAdded' && msg.params.entry && msg.params.entry.timestamp) {
                ts = msg.params.entry.timestamp;
            }
...
async function main() {
    try {
        ...
        // Filter: Keep only logs from the last 60 seconds
        const cutoff = Date.now() - 60000;
        const recentLogs = LOGS.filter(l => new Date(l.timestamp).getTime() > cutoff);

        fs.writeFileSync(OUTPUT_FILE, JSON.stringify(recentLogs, null, 2));
        console.log(`Saved ${recentLogs.length} log entries to ${OUTPUT_FILE} (filtered to last 60s)`);
